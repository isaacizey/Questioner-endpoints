...
def create_meetup():
    #try:
        ...
        new_meetup =meetups_model.MeetupModel().add_meetup
        (data['meetup_location'], data['meetup_happeningOn'], 
        data['meetup_topics'],  data['tags'])
        return jsonify({"status": 201,
         "message": "New meetup created successfully!", "data": new_meetup})
...
def all_meetups():
    #  try:
        ...
        "message" : "Sorry, we could not find any meetups" })
...
def post_rsvp(meetup_id):
    ...
    new_rsvp = meetups_model.RSVPModel().rsvp_meetup(data['meetup'], 
            data['topic'], data['status'])
    if not new_rsvp:
            return jsonify({"status": 404, "message": "No such meetup found!"})

    return jsonify({"status": 201, "message": "New rsvp created successfully!", 
            "data": new_rsvp})
